 ...
 *
 */
public class EditDisturbinActivity extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_edit_disturb);
		...
		position = b.getInt("DATA");
		intWeekper = b.getInt("intWeekper");
		...
		ctvs2 = new CircleTextView[]{ctv1,ctv2,ctv3,ctv4,ctv5,ctv6,ctv7};
		
		
		strStart = list.get(position).getsTime();
		strEnd = list.get(position).geteTime();
		tvStart.setText(timeString(strStart));
		tvEnd.setText(timeString(strEnd));
		
		
		String weekper = Integer.toBinaryString(intWeekper);
		System.out.println("XXX"+weekper);
		char[] b2=weekper.toCharArray();
		for(int i = weekper.length()-1;i>=0;i--){
			if("1".equals(String.valueOf(b2[i]))){
				//sb.append(weeks[weekper.length()-1-i]+" ");
				ctvs2[weekper.length()-1-i].setChecked(true);
			}
		}
...
//		
	...
	public void onClick(View view) {
		...
		case R.id.tv_start:
             ...
             */
            TimePickerDialog timePickerDialog = new TimePickerDialog(EditDisturbinActivity.this, new TimePickerDialog.OnTimeSetListener()
            {
                @Override
		...
		case R.id.tv_end:
             ...
             * 第二个参数是一个TimePickerDialog.OnTimeSetListener匿名内部类，当用户选择好时间后点击done会调用里面的onTimeset方法
             */
            TimePickerDialog timePickerDialog2 = new TimePickerDialog(EditDisturbinActivity.this, new TimePickerDialog.OnTimeSetListener()
            {
                @Override
                public void onTimeSet(TimePicker view, int hourOfDay, int minute)
                {
		...
		if (NetUtils.isNetworkAvailable(this)) {
			...
			executorService.execute(new Runnable() {
				...
				public void run() {
					...
					String str = "";
					times[position] =strStart+"-"+strEnd;
