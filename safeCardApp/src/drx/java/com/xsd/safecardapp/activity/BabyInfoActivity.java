...
public class BabyInfoActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(bundle!=null){
			...
			requestBabyInfo(MainTabActivity.getmResult().get(position).getImei(),0);
		}else{
			try {
				...
				if(!TextUtils.isEmpty(MainTabActivity.getmResult().get(HomePageFragment.oldPosition).getImei())){
					type = 0;
					requestBabyInfo(MainTabActivity.getmResult().get(HomePageFragment.oldPosition).getImei(),0);
				}else{
					if(!TextUtils.isEmpty(MainTabActivity.getmResult().get(HomePageFragment.oldPosition).getUserId())) {
						type = 1;
						requestBabyInfo(MainTabActivity.getmResult().get(HomePageFragment.oldPosition).getUserId(), 1);
					}else{
						type = 2;
					}
				}
			}catch (Exception e){
				Log.e("wangmin",e == null?"e == null":e.getMessage());
			}
	...
	public void updateInfo(View v) {
		...
		if (position != -1) {
			...
			String ime = type == 0?MainTabActivity.getmResult().get(position).getImei():MainTabActivity.getmResult().get(position).getUserId();
			requestUpdateBabyInfo(ime);
		}
	}
	...
	private void requestBabyInfo(final String imei,final int type) {
