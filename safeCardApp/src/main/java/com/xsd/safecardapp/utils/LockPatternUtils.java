...
public class LockPatternUtils {
	...
	private static String LOCK_PATTERN_FILE = "gesture.key";
	/**
    ...
    public void clear() {
        LOCK_PATTERN_FILE = "gesture.key";
        sLockPatternFilename = null;
        sPasswordObserver = null;
    }
	...
	public LockPatternUtils(Context context) {
		if (sLockPatternFilename == null) {
            ...
            SharedPreferences _preferences = context.getSharedPreferences("config", Context.MODE_PRIVATE);
            Consts.USERNAME = _preferences.getString("username", "");
            
			sLockPatternFilename = new File(dataSystemDirectory
					, LOCK_PATTERN_FILE);

            LOCK_PATTERN_FILE = Consts.USERNAME + "_" + LOCK_PATTERN_FILE; // 设置手势文件名称 2015/4/23 14:38
            if (sLockPatternFilename.exists()) { // 做兼容 2015/4/23 14:43
                File nFile = new File(sLockPatternFilename.getParent() + "/" + LOCK_PATTERN_FILE);
                if (sLockPatternFilename.renameTo(nFile)) {// 修改文件名
                    
                    sLockPatternFilename = new File(dataSystemDirectory, LOCK_PATTERN_FILE);
                }
            } else { // 不存在，则直接生成新的手势文件
                sLockPatternFilename = new File(dataSystemDirectory, LOCK_PATTERN_FILE);
            }
